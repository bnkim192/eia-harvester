...
대상: MICHIGAN.HUB (MISO 미시간 트레이딩 허브)."""
...
BACKFILL_START = os.environ.get("MISO_START", "2026-01-01")
...
def parse_day(text, diag=False):
    ...
    for r in rows[hi + 1:]:
        if not r or len(r) <= (max(he) if he else 0):
            continue
        node = r[node_j].strip()
...
def main():
    acc, last = load_state()
    need = (not last) or (not acc) or any(n not in acc for n in NODES)
    if need:
        acc = {}
    ...
    print(f"[RANGE] {start} ~ {end} (last={last}, need_backfill={need})", flush=True)
    d = start
    fetched = 0
    first = True
    while d <= end:
    ...
    out = {"source": "MISO da_expost_lmp monthly avg LMP", "unit": "$/MWh",
           "nodes": NODES, "last_date": end.isoformat(), "acc": acc, "monthly": monthly}
    if not any(monthly.values()):
        print("!! 월별 0 — [Hub 노드 목록] 보고 NODES 수정.", flush=True)
