...
■ v2 수정 (2026-08-11) — 진단 불능 상태를 없앤다
  · 줄 끝 백슬래시(줄이음) 제거. `\` 뒤에 공백 한 칸만 붙어도 SyntaxError 라 모듈이
    컴파일 단계에서 죽고 **산출 파일이 하나도 안 생겨 원인 추적이 불가능**했다.
  · main() 을 try/except 로 감싸 **어떤 런타임 오류에도 eia_rto_monthly.json 에
    traceback 을 남긴다**(probe.fatal). 파일 부재로 헤매는 상황을 구조적으로 차단.
...
def dump_probe(adopted=None, extra=None):
    ...
    out = {"source": "EIA v2 rto (daily)", "adopted": adopted, "probe": PROBE}
    if extra:
        out.update(extra)
...
def route_freqs(key, route):
    ...
    # facets·data 는 EIA 응답에서 dict 일 때도 list 일 때도 있다.
    # 줄이음 백슬래시 없이 분기한다(백슬래시는 뒤 공백 한 칸에 SyntaxError).
    fac = resp.get("facets")
